...
from core.model_fusion_auto import Network_MM_SearchedFusion, Network_MM_Searched, Network_Fusion_Searched
...
from fusion_model.AUIF import DID
from fusion_model.Reconet import ReCoNet
from util.util import compute_results, visualize
from sklearn.metrics import confusion_matrix

from core.model_fusion_auto import Network_MM_CompModel as Network
...
def val_segformer_robust2(model, strategy):
    ...
    # mask_path = './test_all/Mask/'
    # fusion_path = './fusion_res/'
    ...
    with torch.no_grad():
        for attack_mode in ['both']:  # 'both', 'ir', 'vis'
            for _, (images_vis, images_ir, label, name) in tqdm(enumerate(test_loader)):
                ...
                save_root = './attack/{6}_{0}_{1}{2}_{3}_{4}_{5}/'.format(args.attack_loss, args.attack_way,
                                                                          args.attack_iters, int(args.epsilon * 255),
                                                                          int(args.alpha * 255), attack_mode, strategy)
                if attack_mode != 'both':
                    ...
                    os.makedirs(fused_path, mode=0o777, exist_ok=True)
                    os.makedirs(seg_path, mode=0o777, exist_ok=True)
                # print("### Start attack pic {0}".format(name))

                if attack_mode == 'both':
                    # d_ir, d_vi = attack_both(model, X_vis=images_vis, X_ir=images_ir, label=label,
                    #                          attack_loss=args.attack_loss, attack_iters=args.attack_iters,
                    #                          epsilon=args.epsilon, alpha=args.alpha)
                    vi_attacked = images_vis
                    ir_attacked = images_ir
                    ## save attacked image
                ...
                conf = confusion_matrix(y_true=label, y_pred=prediction, labels=[0, 1, 2, 3, 4, 5, 6, 7,
                                                                                 8])  # conf is an n_class*n_class matrix, vertical axis: groundtruth, horizontal axis: prediction
                conf_total += conf
            ...
            res_path = '{0}_PGD{1}_{2}_{3}.txt'.format(strategy, args.attack_iters, int(args.epsilon * 255),
                                                       int(args.alpha * 255))
            with open(save_root + res_path, 'w') as f:
                print("\n strategy :" + strategy, file=f)
                print(
                    "\n Attack Loss{0}, Attack way{1}, Attack iters{2}, epsilon={3}, alpha={4}".format(args.attack_loss,
                                                                                                       'PGD',
                                                                                                       args.attack_iters,
                                                                                                       int(
                                                                                                           args.epsilon * 255),
                ...
                print(
                    ...
                    % (precision_per_class[0], precision_per_class[1], precision_per_class[2], precision_per_class[3],
                       precision_per_class[4],
                       precision_per_class[5], precision_per_class[6], precision_per_class[7], precision_per_class[8],
                       np.mean(np.nan_to_num(precision_per_class))), file=f)
                print(
                    "* iou per class: \n    %.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f" \
                    % (
                        iou_per_class[0], iou_per_class[1], iou_per_class[2], iou_per_class[3], iou_per_class[4],
                        iou_per_class[5],
                        iou_per_class[6], iou_per_class[7], iou_per_class[8], np.mean(np.nan_to_num(iou_per_class))),
                    file=f)
                print("* average values (np.mean(np.nan_to_num(x))) remove unlabel: \n ACC: %.6f, iou: %.6f" \
                      % (np.mean(np.nan_to_num(precision_per_class[1:])), np.mean(np.nan_to_num(iou_per_class[1:]))),
                      file=f)
...
def val_segformer_model(model, strategy):
  conf_total = np.zeros((9, 9))
  h = 480
  w = 640
  model.eval().cuda()
  vi_path = './test_all/Visible/'
  ir_path = './test_all/Infrared/'
  label_path = './test_all/Label/'
  # fusion_path = './fusion_res/'
  # mask_path = './test_part/Mask/'
  ...
  with torch.no_grad():
    for _, (images_vis, images_ir, label,name) in tqdm(enumerate(test_loader)):
      ...
      seg_map = F.interpolate(seg1, size=label.shape[1:], mode='bilinear', align_corners=False)
      label = label.cpu().numpy().squeeze().flatten()
      prediction = seg_map.argmax(
          1).cpu().numpy().squeeze().flatten()  # prediction and label are both 1-d array, size: minibatch*640*480
      conf = confusion_matrix(y_true=label, y_pred=prediction, labels=[0, 1, 2, 3, 4, 5, 6, 7, 8])  # conf is an n_class*n_class matrix, vertical axis: groundtruth, horizontal axis: prediction
      conf_total += conf

    precision_per_class, _, iou_per_class = compute_results(conf_total)
    save_root = './checkpoint/test_result/'
    res_path = 'strategy_{0}_segRes.txt'.format(strategy)
    with open(save_root + res_path, 'w') as f:
      print("\n strategy :" + strategy, file=f)
      print(
          "*precision_per_class and mean: \n    %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f" \
          % (precision_per_class[0], precision_per_class[1], precision_per_class[2], precision_per_class[3], precision_per_class[4],
            precision_per_class[5], precision_per_class[6], precision_per_class[7], precision_per_class[8], np.mean(np.nan_to_num(precision_per_class))), file=f)
      print(
          ...
          iou_per_class[0], iou_per_class[1], iou_per_class[2], iou_per_class[3], iou_per_class[4], iou_per_class[5],
          iou_per_class[6], iou_per_class[7], iou_per_class[8], np.mean(np.nan_to_num(iou_per_class))), file=f)
      print("* average values (np.mean(np.nan_to_num(x))) remove unlabeled: \n ACC: %.6f, iou: %.6f" \
          % (np.mean(np.nan_to_num(precision_per_class[1:])), np.mean(np.nan_to_num(iou_per_class[1:]))), file=f)
    ...
    print(
        "*precision_per_class and mean: \n    %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f \t %.6f" \
        % (precision_per_class[0], precision_per_class[1], precision_per_class[2], precision_per_class[3], precision_per_class[4],
          precision_per_class[5], precision_per_class[6], precision_per_class[7], precision_per_class[8], np.mean(np.nan_to_num(precision_per_class))))
    print(
        "* iou per class and mean: \n    %.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f\t%.6f" \
        % (
        iou_per_class[0], iou_per_class[1], iou_per_class[2], iou_per_class[3], iou_per_class[4], iou_per_class[5],
...
def val_fusion_model(model):
  model.eval().cuda()
  mode = False

  save_root = './fusion_test/'
  if mode:  # 测试正常的图像
    vi_path = './test_all/Visible/'
    ir_path = './test_all/Infrared/'
    label_path = './test_all/Label/'
    fused_path = save_root + 'fused_clear/'

  else:  # 测试被攻击的图像
    vi_path = './attack_dataset/PGD_vis_4_1/'
    ir_path = './attack_dataset/PGD_ir_4_1/'
    label_path = './attack_dataset/Label/'
    fused_path = save_root + 'fused_PGD_4_1/'
  ...
  with torch.no_grad():
    for _, (images_vis, images_ir, label,name) in tqdm(enumerate(test_loader)):
      ...
      image_fusion = model(images_ir, images_vis)
      images_vis_ycrcb = RGB2YCrCb(images_vis) 
      fusion_ycrcb = torch.cat(
          (image_fusion, images_vis_ycrcb[:, 1:2, :, :], images_vis_ycrcb[:, 2:, :, :]),
          dim=1,
...
def val_fusion_train(model, epoch):
    h = 480
    w = 640
    ...
    vi_path = './test_feature/Visible/'
    ir_path = './test_feature/Infrared/'
    label_path = '/root/data1/cvpr_seg/train_all2/Label/'
    mask_path = './test_feature/Mask2/'
    ...
    with torch.no_grad():
        for index in range(0,len(filelist)):
            ...
            name = filelist[index]
            if args.gpu >= 0:
                image_ir = np.array(Image.open(ir_path + name))
                image_ir2 = image_ir
                if len(np.shape(image_ir)) ==2:
                    print()

                    image_ir = image_ir[:, :, np.newaxis]
                    image_ir = np.concatenate([image_ir, image_ir, image_ir], axis=2)
                image_ir = (
                            np.asarray(Image.fromarray(image_ir), dtype=np.float32).transpose(
                                (2, 0, 1)
                            )
                            / 255.0
                    )
                image_ir = np.expand_dims(image_ir, axis=0)
                image_ir = torch.tensor(image_ir).cuda()
                image_vis = np.array(Image.open(vi_path + name))
                image_vis2 = image_vis
                image_vis = (
                ...
                image_vis = torch.tensor(image_vis).cuda()

                # out0, out1 = model2.denoise_net.encoder.forward_fusion(image_mask-0.1)
                image_fusion,ir_feature, vis_feature, lf_ir, hf_ir,res_ir,  lf_vis, hf_vis, res_vis = model.enhance_net.forward2(image_ir,image_vis)
                attentionlist = []
                attentionlist.append(ir_feature)
                attentionlist.append(vis_feature)
                attentionlist.append(lf_ir)
                attentionlist.append(lf_vis)
                attentionlist.append(hf_ir)
                attentionlist.append(hf_vis)
                attentionlist.append(res_ir)
                attentionlist.append(res_vis)
                images_vis_ycrcb = RGB2YCrCb(image_vis)
                fusion_ycrcb = torch.cat(
                    (image_fusion, images_vis_ycrcb[:, 1:2, :, :], images_vis_ycrcb[:, 2:, :, :]),
                    dim=1,
                )
                ...
                for k in range(1):
                    image = fused_image[k, :, :, :]
                    image = Image.fromarray(image)
                    save_path = os.path.join('./test_feature/ours/', name)
                    if not os.path.exists('./test_feature/ours/'+name.split('.')[0]):
                        os.mkdir('./test_feature/ours/'+name.split('.')[0])
                    save_path = './test_feature/ours/'+name.split('.')[0]+'/'
                    image.save(save_path+name)

                    image_ir2 = Image.fromarray(image_ir2)
                    image_vis2 = Image.fromarray(image_vis2)
                    image_ir2.save(save_path+name+'_ir.png')
                    image_vis2.save(save_path+name+'_vis.png')
                    ...
                    for attention in attentionlist:
                        ...
                        attention = attention[0].transpose((1, 2, 0))
                        print('shape',np.shape(attention))
                        attention =np.max(attention, axis=2)
                        visulize_spatial_attention(attention,save_path+str(index_)+'_'+str(0)+'.png')
                        index_+=1
...
import numpy as np
def visulize_spatial_attention(mask,image_pth, ratio=1, cmap="coolwarm"):
    """
    ...
    plt.imshow(normed_mask, alpha=1, interpolation='nearest', cmap=cmap)
    plt.savefig(image_pth,bbox_inches='tight',pad_inches=0.0)
    plt.clf()
...
if __name__ == '__main__':
    ...
    fusion_model_path = './model_Proposed_wodenfense_fusion_best.pth'
    model_fusion  = Network_Fusion_Searched(32,None,fusion_at).cuda()
    model = Network(model_fusion, None
                    , None, 'mit_b3', 9, 256,
                    True).cuda()
    # model = Network_MM_TARDAL(None,None,cfg.exp.backbone,num_classes=9)
    model.load_state_dict(torch.load(fusion_model_path), strict=True)
    print("---------model load done!-----------------")
    print("-----start testing model-----")
    val_segformer_robust2(model,'our_orignal')
